...
// CONFIGURATION
// ==========================================================
#define BACKEND_WS_URL "wss://speaker.yhealth.me/ws/live"
#define DOCTOR_ID "6788b62688a19b5546fe8723"
...
#define VAD_WAKE_THRESHOLD_RMS                                                 \
  150.0f // Standby threshold to trigger session (calibrated for 24dB mic)
#define VAD_ACTIVE_THRESHOLD_RMS 90.0f // Threshold to keep streaming speech
#define MIN_SPEECH_DURATION_MS                                                 \
  250 // Minimum speech required to trigger Gemini turn
#define SILENCE_TIMEOUT_MS                                                     \
  380 // 380ms silence ends utterance -> ultra-fast Gemini response
...
typedef enum {
  CONV_STATE_STANDBY =
      0, // Standby: Soft breathing blue. Listening for wake word.
  CONV_STATE_LISTENING, // Active listening: Bright Cyan. Streaming live PCM to
                        // Gemini.
  CONV_STATE_THINKING,  // Thinking: Amber Yellow. Utterance ended, waiting for
                        // response.
  CONV_STATE_SPEAKING   // Speaking: Vibrant Green. Playing Gemini's audio
                        // response.
} conv_state_t;
...
#define PLAYBACK_PREBUFFER_BYTES                                               \
  6400 // ~200ms audio cushion for crystal-clear fluent playback
...
static void flush_playback_ringbuffer(void) {
  ...
  if (s_audio_play_rb) {
    size_t dummy_size = 0;
    uint8_t *dummy = NULL;
    while ((dummy = (uint8_t *)xRingbufferReceive(s_audio_play_rb, &dummy_size,
                                                  0)) != NULL) {
...
static void led_animation_task(void *pvParameters) {
  ...
  while (1) {
    ...
    case CONV_STATE_THINKING: {
      // Thinking / Reasoning: Soft Pastel Peach / Warm Cream breathing on 3
      // center LEDs
    ...
    case CONV_STATE_SPEAKING: {
      // Assistant Speaking: Soft Pastel Lavender / Lilac wave across 3 center
      // LEDs
...
static void audio_playback_task(void *pvParameters) {
  ...
  while (1) {
    // Pre-buffering phase to absorb Wi-Fi network jitter and eliminate
    // micro-stutters
    if (s_is_prebuffering) {
      if (s_buffered_bytes >= PLAYBACK_PREBUFFER_BYTES ||
          (s_turn_complete && s_buffered_bytes > 0)) {
      ...
      if (s_playback_ctx.is_playing) {
        ...
        if (s_turn_complete || empty_stall_ms >= 200) {
          ...
          if (s_conv_state == CONV_STATE_SPEAKING) {
            ...
            ESP_LOGI(TAG, "🗣️ Assistant finished speaking. Ready for user's "
                          "next question!");
          }
        }
      } else {
    ...
    if (data->op_code == 0x01) { // Text JSON frame
      ...
      if (data->data_len > 0 && s_audio_play_rb) {
        ...
        if (res == pdTRUE) {
          ...
          ESP_LOGW(TAG, "Audio play ring buffer full, dropped chunk (%d bytes)",
                   data->data_len);
        }
      }
    }
...
static esp_err_t ensure_websocket_connected(ws_playback_ctx_t *ctx) {
  if (s_persistent_ws_client != NULL && s_ws_connected &&
      esp_websocket_client_is_connected(s_persistent_ws_client)) {
    return ESP_OK;
  }

  char ws_url[160];
  snprintf(ws_url, sizeof(ws_url), "%s/%s", BACKEND_WS_URL, DOCTOR_ID);
  ESP_LOGI(TAG, "Establishing persistent Live WebSocket connection to: %s",
           ws_url);
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to start persistent WebSocket: %s",
             esp_err_to_name(err));
    return err;
  }
...
static void continuous_mic_stream_task(void *pvParameters) {
  ...
  int chunk_raw_bytes =
      chunk_samples * CHANNELS * sizeof(int32_t); // 4096 bytes
  ...
  ESP_LOGI(
      TAG,
      "🎙️ Continuous Mic Streaming Engine Started. Listening for Wake Word...");
  ...
  while (1) {
    ...
    // 1. If Assistant is currently playing response (or within 500ms cooldown),
    // mute mic to eliminate acoustic feedback
    ...
    if (s_conv_state == CONV_STATE_STANDBY) {
      if (rms >= VAD_WAKE_THRESHOLD_RMS) {
        ESP_LOGI(TAG,
                 "🗣️ Wake Word / Speech Detected! (RMS: %.1f). Activating Live "
                 "Session...",
                 rms);
        s_conv_state = CONV_STATE_LISTENING;
        update_led_state(CONV_STATE_LISTENING);
        ...
        if (s_ws_connected && s_persistent_ws_client &&
          ...
          if (preroll_count > 0) {
            esp_websocket_client_send_bin(
                s_persistent_ws_client, (const char *)preroll,
                preroll_count * sizeof(int16_t), pdMS_TO_TICKS(200));
          }
          // Send current chunk
          esp_websocket_client_send_bin(s_persistent_ws_client,
                                        (const char *)chunk_mono,
                                        CHUNK_MONO_BYTES, pdMS_TO_TICKS(200));
        }
    ...
    if (s_conv_state == CONV_STATE_LISTENING) {
      if (rms >= VAD_ACTIVE_THRESHOLD_RMS) {
        ...
        if (s_ws_connected && s_persistent_ws_client &&
            esp_websocket_client_is_connected(s_persistent_ws_client)) {
          int send_ret = esp_websocket_client_send_bin(
        ...
        if (speech_accum_ms > 0) {
          ...
          // Check if utterance is complete (at least 350ms of speech and 600ms
          // of silence)
          if (speech_accum_ms >= MIN_SPEECH_DURATION_MS &&
              silence_accum_ms >= SILENCE_TIMEOUT_MS) {
            ESP_LOGI(TAG,
                     "⏹️ Utterance complete (Speech: %d ms, Silence: %d ms). "
                     "Triggering Gemini response...",
                     speech_accum_ms, silence_accum_ms);

            const char *eot_msg = "{\"event\":\"audio_end\"}";
            if (s_ws_connected && s_persistent_ws_client &&
                esp_websocket_client_is_connected(s_persistent_ws_client)) {
              esp_websocket_client_send_text(s_persistent_ws_client, eot_msg,
                                             strlen(eot_msg),
                                             pdMS_TO_TICKS(500));
            }
          ...
          if ((now_ms - s_last_speech_time_ms) >
              (IDLE_STANDBY_TIMEOUT_S * 1000)) {
            ESP_LOGI(TAG,
                     "⏳ Session idle timeout (%d seconds). Returning to "
                     "Standby mode.",
                     IDLE_STANDBY_TIMEOUT_S);
...
static void gpio_button_task(void *pvParameters) {
  // Enable pullups on BOOT button GPIO pin (Do NOT touch GPIO 38 which is
  // reserved for WS2812 RGB LED strip)
  ...
  while (1) {
    ...
    if (boot_pressed && prev_boot_state == 1) {
      ...
      if (s_conv_state == CONV_STATE_STANDBY) {
        ...
        ESP_LOGI(TAG,
                 "🔘 BOOT Button Pressed -> Conversation STOPPED (Standby)");
        s_conv_state = CONV_STATE_STANDBY;
    ...
    if (tca_val != prev_tca_val && tca_val != 0xFFFF) {
      ...
      if ((port1 & 0x08) == 0 && (prev_port1 & 0x08) != 0) {
        if (s_conv_state == CONV_STATE_STANDBY) {
          ESP_LOGI(TAG,
                   "🔘 Waveshare Button 3 Pressed -> Conversation ACTIVATED!");
          s_conv_state = CONV_STATE_LISTENING;
          s_last_speech_time_ms = esp_timer_get_time() / 1000;
          update_led_state(CONV_STATE_LISTENING);
        } else {
          ESP_LOGI(TAG, "🔘 Waveshare Button 3 Pressed -> Conversation STOPPED "
                        "(Standby)");
          s_conv_state = CONV_STATE_STANDBY;
          flush_playback_ringbuffer();
          update_led_state(CONV_STATE_STANDBY);
...
void app_main(void) {
  ...
  // Pin audio playback to dedicated Core 1 (Priority 10) isolated from Wi-Fi
  // interrupts
  xTaskCreatePinnedToCore(audio_playback_task, "audio_play_task", 8192, NULL,
                          10, NULL, 1);

  // Pin button listener to Core 0 (Priority 4)
  xTaskCreatePinnedToCore(gpio_button_task, "gpio_button_task", 3072, NULL, 4,
                          NULL, 0);
  ...
  xTaskCreatePinnedToCore(led_animation_task, "led_anim_task", 3072, NULL, 3,
                          NULL, 0);
  s_boot_anim_done = true;
  update_led_state(s_conv_state);

  ESP_LOGI(TAG, "✅ Free Heap Memory: %lu bytes",
           (unsigned long)esp_get_free_heap_size());
  ESP_LOGI(TAG, "System Ready! Continuous Voice Assistant active.");

  // 4. Launch Continuous Microphone Stream Task pinned to dedicated Core 1
  // (Priority 9)
